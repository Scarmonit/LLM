...
/**
 * AWS CLI MCP Server with OpenTelemetry Tracing
...
// Initialize tracing
tracing.initialize({
  serviceName: 'aws-cli-mcp-server',
...
class AWSCliMCPServer {
  constructor() {
    ...
    logger.info('AWS CLI MCP Server initialized with tracing enabled');
  }
  ...
  /**
   * Execute AWS CLI command with tracing
   */
  async executeAWSCommand(service, command, args = [], options = {}) {
    return traceTool(`aws_cli_execute`, { service, command, args, options }, async () => {
      try {
        const {
          profile = 'default',
  ...
  /**
   * Setup MCP tool handlers with tracing
   */
  setupToolHandlers() {
    // List available tools
    this.server.setRequestHandler(ListToolsRequestSchema, async () => 
      traceRequest('list_tools', {}, async () => ({
        tools: [
          {
            name: 'aws_execute',
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) =>
      traceRequest('call_tool', { toolName: request.params.name }, async () => {
  ...
  async run() {
    ...
    logger.info('AWS CLI MCP Server running on stdio with tracing');
  }
}
